...
*/
    addedDevices.push_back(new Device(IDs{255, 253, 254, 255, 255, 255}, msgConv->qstringToString(uiAddress.toString())));
...
//    devices[client] = 0;
    emit stopBroadcast();
  } else if (newDevice->get_IP().length() > 0) {
...
void Server::readyRead() {
  ...
  if (client->canReadLine()) {                              // if we can read from the socket
   ...
   // std::map<QTcpSocket*, int>* ptr_socketmap = &deviceMap;  // ptr for devices map needed for msg transfer, get socket by device ID
    msgHandler->makeCommand(addedDevices, client, msgBytes, deviceMap, msgConv);
